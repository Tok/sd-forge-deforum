...
echo ""
echo -e "${BB0_ZENITH}Mono Variants (Preferred for depth_warp mode):${NC}"
echo "  Size: Small (120MB) → Base (390MB, 3.3×) → Large (1.4GB, 11.7×)"
echo "  1) DA3MONO-Small - Fast, lowest quality"
echo "  2) DA3MONO-Base - Balanced speed/quality"
echo "  3) DA3MONO-Large - Highest quality, slowest"
echo ""
echo -e "${BB0_VOID}AnyView Variants (Required for da3_multiview mode):${NC}"
echo "  Size: Small (120MB) → Base (390MB, 3.3×) → Large (1.4GB, 11.7×)"
echo "  4) AnyView-Small - Multi-view geometry + ray maps"
echo "  5) AnyView-Base - Multi-view geometry + ray maps"
echo "  6) AnyView-Large - Multi-view geometry + ray maps"
echo ""
echo -e "${BB0_GLITCH}Giant Variant (ONLY for DA3-3DGS interpolation):${NC}"
echo "  Size: 4.6GB (38× larger than Small, 1.15B params)"
echo "  7) DA3-GIANT - 3D Gaussian Splatting (Keyframes + Interpolation mode only)"
echo ""
...
echo "  • Mono: Preferred for tween_generation_mode='depth_warp' (default)"
echo "  • AnyView: Required for tween_generation_mode='da3_multiview'"
echo "  • Giant: ONLY for Keyframes+Interpolation mode → DA3-3DGS interpolation method"
echo "           (NOT selectable in 3D Depth tab, separate da3_3dgs_model parameter)"
echo ""
