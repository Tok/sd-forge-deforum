...
echo ""
echo -e "${BB0_ZENITH}Mono Variants (For 3D mode depth warping):${NC}"
...
echo ""
echo -e "${BB0_VOID}AnyView Variants (Required for DA3-Multiview interpolation):${NC}"
echo "  Size: Small (120MB) → Base (390MB, 3.3×) → Large (1.4GB, 11.7×)"
...
echo "  7) DA3-GIANT - 3D Gaussian Splatting reconstruction"
echo ""
...
echo "  • Mono: For 3D mode → tween_generation_mode='depth_warp' (classic)"
echo "  • AnyView: For Keyframes+Interpolation mode → DA3-Multiview method (auto-loaded)"
echo "  • Giant: For Keyframes+Interpolation mode → DA3-3DGS method (auto-loaded)"
echo "           (Giant is NOT for depth warping, ONLY for Gaussian Splatting)"
echo ""
