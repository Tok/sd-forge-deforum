    ...
    for setting_name, deprecation_info in deprecation_map.items():
        if setting_name in settings_json:
            if deprecation_info is None:
                logger.warning(f"Setting '{setting_name}' has been removed. It will be discarded and the default value used instead!")
                ...
                if new_value is not None:
                    logger.warning(f"Setting '{setting_name}' has been renamed to '{new_setting_name}' with value '{new_value}'. The saved settings file will reflect the change")
                ...
                if isinstance(old_value, (int, float)):
                    ...
                    logger.warning(f"Value '{old_value}' for setting '{setting_name}' has been replaced with '{new_value}'. The saved settings file will reflect the change")
                    settings_json[setting_name] = new_value
            elif isinstance(deprecation_info, str):
                logger.warning(f"Setting '{setting_name}' has been renamed to '{deprecation_info}'. The saved settings file will reflect the change")
                ...
                for old_value, new_value, is_removed in deprecation_info:
                    if settings_json[setting_name] == old_value:
                        if is_removed:
                            logger.warning(f"Value '{old_value}' for setting '{setting_name}' has been removed. It will be discarded and the default value used instead!")
                        else:
                            logger.warning(f"Value '{old_value}' for setting '{setting_name}' has been replaced with '{new_value}'. The saved settings file will reflect the change")
                            settings_json[setting_name] = new_value