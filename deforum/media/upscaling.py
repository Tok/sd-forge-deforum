...
def process_ncnn_video_upscaling(vid_path, outdir, in_vid_fps, in_vid_res, out_vid_res, models_path, upscale_model, upscale_factor, keep_imgs, f_location, f_crf, f_preset, current_user_os):
    ...
    _get_logger().info("\r" + " " * len(msg_to_print), end="", flush=True)
    _get_logger().info(f"\r{msg_to_print}", flush=True)
    _get_logger().info(f"\rUpscaling \033[0;32mdone\033[0m in {time.time() - start_time:.2f} seconds!", flush=True)
