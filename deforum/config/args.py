...
def RootArgs():
    return {
        ...
        "half_precision": not getattr(sh.cmd_opts, 'no_half', False),  # Forge Neo removed this flag
        "clipseg_model": None,
...
def DeforumAnimArgs():
    return {
        ...
        "distilled_cfg_scale_schedule": {
            "label": "Distilled CFG schedule (Flux only)",
            "type": "textbox",
            "value": "0: (3.5)",
            "info": "Flux: Distilled CFG guidance strength (Schnell/Dev models). Supports full scheduling. Ignored for other models."
        },
        "shift_schedule": {
            "label": "Shift schedule (Z-Image only)",
            "type": "textbox",
            "value": "0: (3.0)",
            "info": "Z-Image: FlowMatch shift parameter controls timestep schedule scaling (range 1.0-7.0, optimal ~3.0). Higher values increase prompt adherence. Supports full scheduling. Ignored for other models."
        },
        ...
        "tween_generation_mode": {
            ...
            "choices": ['depth_warp'],
            "value": "depth_warp",
            "info": "Tween generation: depth_warp (classic depth warping, uses Mono models). For DA3-Multiview (multi-view geometry) or DA3-3DGS (3D Gaussian Splatting), use 'Keyframes + Interpolation' render mode instead."
        },
        "da3_use_ray_pose": {
