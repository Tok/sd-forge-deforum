...
class TestParallaxOrbit:
    """Test parallax-orbit preset path generation."""

    def test_parallax_orbit_basic(self):
        """Test basic parallax orbit path generation."""
        num_frames = 333
        radius = 100.0
        height = 0.0

        camera_path = generate_parallax_orbit_path(num_frames, radius, height)

        assert len(camera_path) == num_frames
        assert camera_path[0].frame == 0
        assert camera_path[-1].frame == num_frames - 1

    def test_parallax_orbit_visible_rotation(self):
        """Test that parallax orbit has VISIBLE rotation (1-2° per frame)."""
        num_frames = 333
        radius = 100.0
        height = 0.0
        ...
        # Calculate average rotation delta per frame (rot_y only - simple pan)
        rotation_deltas = []
        for i in range(1, len(camera_path)):
            delta_rot_y = camera_path[i].rot_y - camera_path[i-1].rot_y
            rotation_deltas.append(abs(delta_rot_y))

        avg_rotation_delta = np.mean(rotation_deltas)

        # Visible rotation should be ~0.65°/frame (216° total / 333 frames)
        # At 60 FPS, this is ~39°/sec - VISIBLE to img2img
        # Compare to rotate-around: ~0.3°/frame - INVISIBLE to img2img
        assert avg_rotation_delta > 0.5, (
            f"Rotation too small ({avg_rotation_delta:.3f}°/frame), won't be visible in preview"
        )
        assert avg_rotation_delta < 3.0, (
            f"Rotation too large ({avg_rotation_delta:.3f}°/frame), will cause jitter"
        )

    def test_parallax_orbit_rotation_speed_ratio(self):
        """Test that rotation is 60% of translation speed (creates parallax)."""
        num_frames = 333
        radius = 100.0
        height = 0.0
        ...
        # Total rotation: Should be 216° (360° * 0.6)
        total_rotation = camera_path[-1].rot_y - camera_path[0].rot_y

        # Allow small tolerance due to float precision
        expected_rotation = 360.0 * 0.6 * (num_frames - 1) / num_frames
        assert abs(total_rotation - expected_rotation) < 1.0, (
            f"Expected ~{expected_rotation:.1f}° rotation (60% of orbit), "
            f"got {total_rotation:.1f}°"
        )

    def test_parallax_orbit_translation_is_circular(self):
        """Test that translation follows circular orbit."""
        num_frames = 333
        radius = 100.0
        height = 0.0
        ...
        # Check XZ distance is constant (circular orbit)
        for point in camera_path:
            distance_xz = np.sqrt(point.x**2 + point.z**2)
            assert abs(distance_xz - radius) < 0.01, (
                f"Frame {point.frame}: Expected radius {radius}, got {distance_xz:.2f}"
            )

        # Check Y is constant (flat orbit)
        for point in camera_path:
            assert abs(point.y - height) < 0.01

    def test_parallax_orbit_level_camera(self):
        """Test that camera stays level (no tilt or roll)."""
        num_frames = 100
        radius = 100.0
        height = 0.0
        ...
        for point in camera_path:
            assert point.rot_x == 0.0, f"Frame {point.frame}: Expected no tilt, got {point.rot_x}°"
            assert point.rot_z == 0.0, f"Frame {point.frame}: Expected no roll, got {point.rot_z}°"

    def test_parallax_orbit_rotation_translation_mismatch(self):
        """Test that rotation doesn't cancel translation (creates parallax, not look-at).

        For rotate-around (look-at center), rotation cancels translation with ~-5 factor.
        For parallax-orbit, rotation is SLOWER than translation to create depth-warp parallax.
        This test verifies the mismatch exists and is intentional.
        """
        num_frames = 333
        radius = 100.0
        height = 0.0
        ...
        # Sample points to check rotation-translation relationship
        for i in range(10, len(camera_path), 50):
            point = camera_path[i]
            prev_point = camera_path[i - 1]

            # Calculate translation deltas
            delta_x = point.x - prev_point.x
            delta_z = point.z - prev_point.z

            # Calculate rotation delta
            delta_rot_y = point.rot_y - prev_point.rot_y

            # For rotate-around (look-at), we'd expect: delta_rot_y ≈ -5 * delta_x (cancels motion)
            # For parallax-orbit, rotation is INDEPENDENT and slower
            # Angular velocity around orbit: ~1.08°/frame (360° / 333 frames)
            # Rotation velocity: ~0.65°/frame (216° / 333 frames) = 60% of orbit

            # Verify rotation is NOT trying to look at center
            # If it were look-at, rotation would be much larger to track the center
            # Angular position around the circle (where camera actually is)
            angular_position = np.arctan2(point.z, point.x)  # atan2(z, x) for XZ plane
            expected_look_at_rotation = np.degrees(angular_position)

            # For parallax-orbit, rot_y is independent (60% of frame progression)
            actual_rotation = point.rot_y

            # Normalize to 0-360 range for comparison
            expected_look_at_rotation = expected_look_at_rotation % 360
            actual_rotation = actual_rotation % 360

            # Calculate absolute difference (accounting for wraparound)
            rotation_diff = abs(expected_look_at_rotation - actual_rotation)
            if rotation_diff > 180:
                rotation_diff = 360 - rotation_diff

            # Parallax-orbit should NOT match look-at rotation (difference should be large)
            # After ~50 frames, the mismatch should be noticeable (40% difference accumulates)
            if i > 50:
                assert rotation_diff > 10.0, (
                    f"Frame {i}: Rotation too close to look-at behavior "
                    f"(diff={rotation_diff:.1f}°, expected_look_at={expected_look_at_rotation:.1f}°, "
                    f"actual={actual_rotation:.1f}°). Should create parallax, not look-at."
                )
